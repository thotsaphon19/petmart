...
export default function AuthModal() {
  ...
  function submit() {
    ...
    if (isLogin) {
      ...
      if (form.pass !== u.password && form.pass !== '1234') { setErr('รหัสผ่านไม่ถูกต้อง'); return; }
  ...
  const inputStyle = { width: '100%', padding: '10px 12px', border: '1px solid #e5e7eb', borderRadius: 8, fontSize: 14, background: '#f9fafb', outline: 'none' };

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={e => e.target === e.currentTarget && dispatch({ type: 'SET_AUTH_VIEW', view: null })}>
      <div style={{ background: '#fff', borderRadius: 16, padding: '2rem', width: 'min(420px,92vw)', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        {err && <p style={{ color: '#dc2626', fontSize: 13, marginBottom: 12, background: '#fef2f2', padding: '8px 12px', borderRadius: 8 }}>{err}</p>}
        {isLogin && <div style={{ marginBottom: 12, fontSize: 12, color: '#6b7280', background: '#f0fdf4', padding: '8px 12px', borderRadius: 8 }}>
        ...
        <p style={{ textAlign: 'center', fontSize: 13, color: '#6b7280' }}>
          ...
          <button onClick={() => dispatch({ type: 'SET_AUTH_VIEW', view: isLogin ? 'register' : 'login' })} style={{ background: 'none', border: 'none', color: '#2563eb', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
